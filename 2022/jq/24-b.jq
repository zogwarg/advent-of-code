...
def go_to($end;$start): last(
  label $out | foreach range(1e9) as $_ (
    ...
    def cost($e):
      if $end == "end" then $e.m + ($H-$e.y) + ($W - 1 - $e.x)
                       else $e.m + $e.y + $e.x end;
    def heap_push($e):
      def   rank($i): $i + 1 | logb;
    ...
    if $end == "end" and $x == ($W - 1) and $y == ($H - 1) then
      .target = { m: $m + 1, $x, y: $y+1, h: $h + "v" }
    elif $end == "start" and [$x, $y] == [0,0] then
      .target = { m: $m + 1, $x, y: $y-1, h: $h + "^" }
    else
      reduce (
        (
          [$x-1,$y,"\($h)<"], [$x+1,$y,"\($h)>"],
...
reduce limit(3;"end","start","end") as $t (
   { pos: { m: 0, x: 0, y: -1, h: "" }};
    .pos = go_to($t;.pos).target
) | .pos | debug({path: .h}) | .m

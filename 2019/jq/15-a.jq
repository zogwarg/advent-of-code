...

{
  ...
  wall: {},
  i: 0
} | # Do BFS search, copying $func state as needed
until (isempty( .bots[] ) or .i >= 5000 or .target; .bots[0] as $bot |
  .bots = .bots[1:] | .i += 1 | reduce(
    (
      $bot.pos
      | (. + [1] | .[1] -= 1 ), (. + [2] | .[1] += 1 ),    # N, S
        (. + [3] | .[0] -= 1 ), (. + [4] | .[0] += 1 )     # W, E
...
if .target then
  .target[1] # Min Distance to O2
else
  "Oxygen not found" | halt_error
end

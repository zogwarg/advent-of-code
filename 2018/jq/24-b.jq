...
] | { real_units: ((.[0]|map(.+["M"])) + (.[1]|map(.+["I"]))) } |


first(
range(5000) as $boost | debug({$boost}) |
{units:(.real_units|map(if .[-1] == "M" then .[2] += $boost end))}|

until (                    .abort == true   or # Kill if no updates
  ...
  | .init |= sort_by(-.[0]) | # Roll for initiative then sort queue
      ...
      .prev = "\(.units)"   | # Save state of units
  ...
  | .abort = ( .prev == "\(.units)" )    # Compare with saved state
) |       select(.abort | not)           # Only keep victories

# Output first sum of remaining units,
# When only the immune system groups are still on the board.
| [ .units[] | select(.[-1]=="M") | .[0] ] | add | select(.)

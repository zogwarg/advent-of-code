...
( $init.units | map(select(.[1] == "E")) | length) as $num_elves |
...
# Get first battle where no elves have died
first(foreach range(3; 50) as $elf_power ({}; $init |

# Until one side has won, abort a given run if elf died
until (([ .units[][1] ] | unique | length == 1 ) or .elf_died;
  def move_unit:
    (
  ...
  def unit_atk:
    ...
    if isempty($i[]) then . else
      ...
      .units[$i][2] -= (if $u[1] == "E" then $elf_power else 3 end) |
      # Remove unit if it dies
      if .units[$i][2] <= 0 then
        if .units[$i][1] == "E" then .elf_died = true end |
        .field["\(.units[$i][0])"] = "." |
